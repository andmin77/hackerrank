...
class NetworkAdministration {
	...
	public void solve(int testNumber, InputReader in, PrintWriter out) {
		...
		for (int i = 0; i < L; i++) {
			...
			if (lct[a][x] == null) lct[a][x] = new LinkCutTree.Node(0);
			if (lct[a][y] == null) lct[a][y] = new LinkCutTree.Node(0);
		...
		for (int i = 0; i < T; i++) {
			...
			if (t == 1) {
				...
				if (prevAdmin == null) {
				...
				} else if (lct[admin][a] != null && lct[admin][b] != null && LinkCutTree.connected(lct[admin][a], lct[admin][b])) {
					...
					if (lct[admin][a] == null) lct[admin][a] = new LinkCutTree.Node(0);
					if (lct[admin][b] == null) lct[admin][b] = new LinkCutTree.Node(0);
				...
				int x = in.nextInt();
				LinkCutTree.Node edgeNode = infos[Arrays.binarySearch(edges, edge(a, b))].edgeNode;
				LinkCutTree.modify(edgeNode, edgeNode, x);
			} else {
				int admin = in.nextInt() - 1;
				if (lct[admin][a] == null || lct[admin][b] == null || !LinkCutTree.connected(lct[admin][a], lct[admin][b])) {
